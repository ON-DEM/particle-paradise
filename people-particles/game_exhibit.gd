...
func _physics_process(delta: float) -> void:
	for particle in attracting.keys():
		...
		particle.apply_central_force(direction.normalized() * (clampf(10-particle.boredom, 0, 10)) * clampf(1.0/particle.global_position.distance_to(global_position), 0.0, 1.0))
