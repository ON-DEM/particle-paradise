...
func _physics_process(delta: float) -> void:
	for particle in attracting.keys():
		...
		particle.apply_central_force(direction.normalized() * (clampf(4.0-particle.boredom, 0, 10)) * clampf(3.0/particle.global_position.distance_to(global_position), 0.0, 3.0))
